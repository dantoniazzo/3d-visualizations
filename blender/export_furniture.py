"""Export the furniture catalogue as individual GLBs.

Every piece is generated by the same scripts the house is built from —
joinery.py for built-ins, loose.py for seating, tables, lamps and decor — so
nothing is downloaded and nothing needs a licence. Each is built at real
size, centred on its footprint and standing on y = 0, which is what the
editor expects; its bounds are what the app collides with.
...
from lib import geometry as g, joinery as j, loose as L, materials as m
...
PIECES = [
    ...
    ("shower", "Shower enclosure", "bathroom",
     lambda c, mats: j.shower(c, mats, 0, 0)),
    ...
    # Loose furniture, from loose.py.
    ("sofa", "Sofa, 3-seat", "seating",
     lambda c, mats: L.sofa(c, mats, 0, 0)),
    ("sofa-2", "Sofa, 2-seat", "seating",
     lambda c, mats: L.sofa(c, mats, 0, 0, w=1.60, seats=2, fabric="fabric_sage")),
    ("armchair", "Armchair", "seating",
     lambda c, mats: L.armchair(c, mats, 0, 0)),
    ("lounge-chair", "Lounge chair", "seating",
     lambda c, mats: L.lounge_chair(c, mats, 0, 0)),
    ("desk-chair", "Desk chair", "seating",
     lambda c, mats: L.office_chair(c, mats, 0, 0)),
    ("dining-chair", "Dining chair", "seating",
     lambda c, mats: L.dining_chair(c, mats, 0, 0)),
    ("bar-stool", "Bar stool", "seating",
     lambda c, mats: L.bar_stool(c, mats, 0, 0)),
    ("ottoman", "Ottoman", "seating",
     lambda c, mats: L.ottoman(c, mats, 0, 0)),
    ("dining-table", "Dining table, 6-seat", "tables",
     lambda c, mats: L.dining_table(c, mats, 0, 0)),
    ("dining-table-4", "Dining table, 4-seat", "tables",
     lambda c, mats: L.dining_table(c, mats, 0, 0, w=1.20, d=0.80)),
    ("coffee-table", "Coffee table", "tables",
     lambda c, mats: L.coffee_table(c, mats, 0, 0)),
    ("coffee-table-round", "Round coffee table", "tables",
     lambda c, mats: L.coffee_table_round(c, mats, 0, 0)),
    ("side-table", "Side table", "tables",
     lambda c, mats: L.side_table(c, mats, 0, 0)),
    ("console-table", "Console table", "tables",
     lambda c, mats: L.console_table(c, mats, 0, 0)),
    ("bookcase", "Bookcase", "storage",
     lambda c, mats: L.bookcase(c, mats, 0, 0)),
    ("tv", "Television", "decor",
     lambda c, mats: L.tv(c, mats, 0, 0)),
    ("laptop", "Laptop", "decor",
     lambda c, mats: L.laptop(c, mats, 0, 0)),
    ("desk-lamp", "Desk lamp", "lighting",
     lambda c, mats: L.desk_lamp(c, mats, 0, 0)),
    ("plant", "Potted plant", "decor",
     lambda c, mats: L.plant(c, mats, 0, 0, h=1.0)),
    ("plant-small", "Small plant", "decor",
     lambda c, mats: L.plant(c, mats, 0, 0, h=0.55)),
    ("vase", "Vase", "decor",
     lambda c, mats: L.vase(c, mats, 0, 0)),
    ("bowl", "Bowl", "decor",
     lambda c, mats: L.bowl(c, mats, 0, 0)),
    ("books", "Books", "decor",
     lambda c, mats: L.books(c, mats, 0, 0)),
    ("picture", "Picture", "decor",
     lambda c, mats: L.picture(c, mats, 0, 0), "wall"),
    ("mirror", "Mirror", "decor",
     lambda c, mats: L.mirror(c, mats, 0, 0), "wall"),
]
...
def main():
    ...
    for piece_id, label, category, build, *rest in PIECES:
        anchor = rest[0] if rest else "floor"
        fresh()
        mats = m.library()
